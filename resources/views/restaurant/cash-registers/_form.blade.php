<div class="space-y-6">
    <div>
        <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nombre de la caja</label>
        <input id="name" name="name" type="text" value="{{ old('name', $cashRegister->name ?? '') }}" required
    ...
    <div>
        <label for="status" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
            <input id="status" name="status" type="checkbox" value="1"
                {{ old('status', $cashRegister->status ?? true) ? 'checked' : '' }}
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('restaurant.cash-registers.index') }}"
