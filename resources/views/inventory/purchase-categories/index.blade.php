...
@section('content')
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Categorías de compra</h1>
            <p class="mt-1 text-sm text-gray-600">Clasifican los productos que se compran (materia prima, insumo).</p>
        </div>
        @can('inventario-ver')
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($purchaseCategories as $purchaseCategory)
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $purchaseCategory->name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $purchaseCategory->description ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $purchaseCategory->products_count }}</td>
