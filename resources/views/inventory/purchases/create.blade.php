    ...
    <div class="mt-6 max-w-3xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <form action="{{ route('inventory.purchases.store') }}" method="POST">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label for="document_type_id" class="block text-sm font-medium text-gray-700">Comprobante</label>
            ...
            <div class="mt-4 grid grid-cols-3 gap-4">
                <div>
                    <label for="purchase_type" class="block text-sm font-medium text-gray-700">Tipo de compra</label>
            ...
            <div class="mt-6">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" data-add-line
                        class="text-sm font-medium text-brand-600 hover:text-brand-500">+ Agregar producto</button>
                </div>
                <div data-lines-list class="mt-2 space-y-2">
                    ...
                    @empty
                        <p data-lines-empty class="text-sm text-gray-500">Agregue al menos un producto a la compra.</p>
                    @endforelse
                </div>
                @error('details')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
...
@push('scripts')
    <script>
        const lineOption = (index) => `
            <select name="details[${index}][product_id]" required class="block w-full rounded-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
                <option value="">Seleccione producto</option>
                @foreach ($supplies as $supply)
                    <option value="{{ $supply->id }}">{{ $supply->name }}</option>
                @endforeach
            </select>
            <input type="number" step="0.01" min="0" name="details[${index}][quantity]" placeholder="Cant." required class="w-24 rounded-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
            <input type="number" step="0.01" min="0" name="details[${index}][unit_price]" placeholder="P. unit." required class="w-28 rounded-md border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500">
            <button type="button" data-remove-line class="text-sm font-medium text-red-600 hover:text-red-500">Quitar</button>
        `;
