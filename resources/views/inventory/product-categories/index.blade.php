...
@section('content')
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Categorías de platos que se muestran en el POS.</p>
        </div>
        @can('inventario-ver')
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                @forelse ($productCategories as $productCategory)
                    <tr>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white">{{ $productCategory->name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $productCategory->description ?? '—' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{{ $productCategory->products_count }}</td>
                        <td class="px-6 py-4">
